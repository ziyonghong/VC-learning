...
TCHAR szWindowClass[MAX_LOADSTRING]=L"ball";			// 主窗口类名
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		hdc = GetDC (hWnd);
		xPixel = GetDeviceCaps(hdc,ASPECTX);// 取回划一条线用的 X 向像素单位
		yPixel = GetDeviceCaps(hdc,ASPECTY); ReleaseDC (hWnd, hdc);
		return 0;
	case WM_SIZE:	// 收到改变窗口尺寸的命令时, 
		xCenter = (cxClient = LOWORD (lParam))/ 2;
		yCenter = (cyClient = HIWORD (lParam))/ 2; // 指定客户区的高度
